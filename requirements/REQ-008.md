...
- [ ] Integration tests cover: every MCP tool with valid inputs (returns expected result)
- [ ] Integration tests cover: every MCP tool with invalid inputs (returns correct error code)
- [ ] Integration tests cover: multi-step workflow — create req → create tasks with dependencies → pick in correct order → complete → verify req status transitions
- [ ] Integration tests cover: ownership enforcement — agent B cannot complete agent A's task
- [ ] Integration tests cover: dependency blocking — pick_task fails when deps not satisfied
